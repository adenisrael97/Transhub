...
import { fetchOperatorDrivers, createDriver } from "@/services/trips";
...
export default function OperatorDriversPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetchOperatorDrivers();
  ...
  function validate() {
    ...
    if (!form.phone.trim() || form.phone.trim().length < 7) {
      setFormError("A valid phone number is required."); return false;
    }
    if (form.password.length < 8) {
      setFormError("Password must be at least 8 characters."); return false;
    }
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[#64748B]">{drivers.length} driver{drivers.length !== 1 ? "s" : ""}</p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" variant="secondary" onClick={load} loading={loading && drivers.length > 0}>
              Refresh
            </Button>
          ...
          <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden">
            ...
            {drivers.length === 0 && !loading && (
              <div className="text-center py-16">
                ...
                <p className="text-sm font-medium text-[#94A3B8]">No drivers yet — add your first driver above</p>
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setFormError(""); }} title="Add New Driver" size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" variant="success" loading={formLoading} className="flex-1">
              Create Driver
            </Button>
          </div>
        </form>
