...
type NewOperatorInput = Omit<Operator, "id" | "status" | "appliedAt" | "reviewedAt">;

interface OperatorState {
  operators: Operator[];
  addOperator: (data: NewOperatorInput) => Operator;
  approveOperator: (id: string) => void;
  declineOperator: (id: string) => void;
...
const useOperatorStore = create<OperatorState>((set, get) => ({
  ...
  approveOperator(id) {
    set((s) => ({
      operators: s.operators.map((o) =>
        o.id === id
          ? { ...o, status: "approved" as OperatorStatus, reviewedAt: new Date().toISOString() }
          : o
      ),
    }));
  },
  ...
  declineOperator(id) {
    set((s) => ({
      operators: s.operators.map((o) =>
        o.id === id
          ? { ...o, status: "declined" as OperatorStatus, reviewedAt: new Date().toISOString() }
          : o
      ),
    }));
  },
