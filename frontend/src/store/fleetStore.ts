...
/**
 * Mock driver shape used by the fleet store (different from backend DriverDTO).
 * This will be replaced when the passenger dashboard connects to the real API.
 */
...
interface FleetState {
  ...
  getVisibleTrips: (filters?: Partial<TripSearchParams>) => Trip[];
}

const useFleetStore = create<FleetState>((set, get) => ({
  drivers: [
    {
      id: "DRV-001",
      name: "Emeka Okafor",
  ...
  getVisibleTrips(filters = {}) {
    ...
    return get().trips.filter((trip) => {
      if (!trip.driverId || !availableDriverIds.has(trip.driverId)) return false;
